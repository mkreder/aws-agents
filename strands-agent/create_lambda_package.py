#!/usr/bin/env python3
"""
Lambda Deployment Package Creator for Strands Agent

This script creates deployment packages for AWS Lambda by:
1. Creating two separate zip files for Lambda deployment:
   - app.zip: Contains the Lambda function code from the 'packaging/lambda' directory
...
4. Both zip files are stored in the 'packaging' directory for SAM to use during deployment
...
def create_lambda_package():
    ...
    print(f"Creating Strands Agent Lambda deployment package: {app_deployment_zip}")
    ...
    with zipfile.ZipFile(dependencies_deployment_zip, 'w', zipfile.ZIP_DEFLATED) as zipf:
        for root, _, files in os.walk(dependencies_dir):
            for file in files:
                ...
                # Put dependencies directly under python/ (remove the dependencies/ part)
                rel_path = os.path.relpath(file_path, dependencies_dir)
                arcname = Path("python") / rel_path
    ...
    print(f"Strands Agent Lambda deployment packages created successfully:")
    print(f"  - {dependencies_deployment_zip.name} ({dependencies_deployment_zip.stat().st_size / 1024 / 1024:.1f} MB)")
    print(f"  - {app_deployment_zip.name} ({app_deployment_zip.stat().st_size / 1024:.1f} KB)")
    return True
